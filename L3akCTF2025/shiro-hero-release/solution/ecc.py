...
class ECDSA:
    ...
    def gen_keypair():
        ...
        d = random.randint(1, ECDSA.n - 1)         # private
        Q = ECDSA.scalar_mult(d, ECDSA.G)          # public
        return d, Q                                 # ← (priv, pub)
    ...
    def ecdsa_sign(h: int, d: int, prng: xorshiro256):
        ...
        while True:
            ...
            if x is None:  # Handle the case where scalar_mult returns (None, None)
                continue
            r = x % ECDSA.n
            if not r:
                continue
    ...
    def ecdsa_verify(h, Q, sig):
        ...
        if x is None:  # Handle the case where point_add returns (None, None)
            return False
        return (x % ECDSA.n) == r
